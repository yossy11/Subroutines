     ...
     & HARDN=0.25D0,HARDSTRAIN0=4.0D-3,YLDM=6)
      ...
      yldCPrime(1,2) = -1.0577581290163252D0
      yldCPrime(1,3) = -0.9687606069158757D0
      yldCPrime(2,1) = -1.0310867453740127D0
      yldCPrime(2,3) = -0.9936450481619784D0
      yldCPrime(3,1) = -0.9954912966094002D0
      yldCPrime(3,2) = -0.966728263261663D0
      yldCPrime(4,4) = 0.9982532697189669D0
      yldCPrime(5,5) = 1.0D0
      yldCPrime(6,6) = 1.0D0

      yldCDbPrime(1,2) = -1.058024565508002D0
      yldCDbPrime(1,3) = -0.9685622219098127D0
      yldCDbPrime(2,1) = -1.0309874986293928D0
      yldCDbPrime(2,3) = -0.9935777476362849D0
      yldCDbPrime(3,1) = -0.995579137351788D0
      yldCDbPrime(3,2) = -0.9667666654084238D0
      yldCDbPrime(4,4) = 0.9981384725399618D0
    ...
    !  &   H*eqGStress/eqStress)
     ...
     & vec2(7),H,C(7,7),subDDSDDE(6,6),ddfddS(6,6),calc_eqStress,YLDM
      subDDSDDE(:,:) = DDSDDE(:,:)
      ! eqGStress = calc_eqGStress(hillParams,STRESS)
      eqGStress = calc_eqStress(YLDM,yldCPrime,yldCDbPrime,STRESS)
      CALL calc_dfdS(YLDM,yldCPrime,yldCDbPrime,STRESS,dfdS)
    ...
    !  & (DOT_PRODUCT(dfdS,MATMUL(B,dGdS)) + H*h0)
     ...
     & ddfddS)
      ...
      INTEGER i
      DOUBLE PRECISION yldCPrime(6,6),yldCDbPrime(6,6),STRESS(6),
     & dfdS(6),ddfddS(6,6),DELTAX,subStress(6),subdfdS(6),YLDM

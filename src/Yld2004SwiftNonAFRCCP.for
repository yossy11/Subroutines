     ...
     & denominatordLambda,dyadMat(6,6),F,H

      ! define constants
      PARAMETER(TOLER=1.0D-6,YOUNG=6.9D4,POISSON=0.33D0,HARDK=646.0D0,
     & HARDN=0.227D0,HARDSTRAIN0=2.5D-2,YLDM=6)
      ...
      DO WHILE (iterationNum < 200)
        ...
        dLambda = F/DOT_PRODUCT(dfdS,MATMUL(DDSDDE,dGdS)) + H + eqGStress/eqStress
     ...
     & HARDN,eqpStrain)
      ...
      IMPLICIT NONE
      DOUBLE PRECISION hillParams(4),STRESS(6),dGdS(6),eqGStress,calc_eqGStress,multiplier
      eqGStress = calc_eqGStress(hillParams,STRESS)
     ...
     & dfdS(6),subStress(6)
      ...
      DO i=1,6
        ...
        dfdS(i) = (calc_eqStress(YLDM,yldCPrime,yldCDbPrime,subStress) - 
     &   eqStress)/DELTAX
      END DO

...
C     HH-HARDENING RATE  HW-FACTOR
      DOUBLE PRECISION SIGMA_X,SIGMA_Y,SIGMA_Z,TAO_XY,TAO_YZ,TAO_ZX,
     & ALPHAF,ALPHAG,NBLFM1,NBLFM2,DNBL,NBLFM,NBLFZ,CNBL,JCOBFM,BBB,GAA
C	
...
C     VARIABLES IN F FUNCTION
      DOUBLE PRECISION FSIGMA_SX,FSIGMA_SY,FSIGMA_SZ,FTAO_SYZ,FTAO_SZX,
     & FTAO_SXY,FEQSTRESS,FSUM,FSA,FSB,FSC,FSD,FSE,FSF
C     VARIABLES IN G FUNCTION
      DOUBLE PRECISION GSIGMA_SX,GSIGMA_SY,GSIGMA_SZ,GTAO_SYZ,GTAO_SZX,
     & GTAO_SXY,GEQSTRESS,GSUM,GH,GF,GG,GL,GM,GN,GSA,GSB,GSC,GSD,GSE,GSF
...
C     NEW VARIABLES (UN-FINISHED)
     ...
     & JCOBFZ(6,6),DDBDDS(6,6),DSTRES(6),DDGDS(6),DDSTIFF(6,6),
     & DDEDDB(6,6),Q(6,6),INVQ(6,6),R(6,6),NBLFMT(6), R0(6),NBLQR(6),
     & ATRT(6),RB(6),IS(6),JS(6),PHI(20),TSTRESS(6),KSTRESS(6),
     & KDSTRES(6),MSTRESS(6),NF(6),NG(6),NDDGDS(6)
      DOUBLE PRECISION EELAS,EPLAS,PSTRES,DDFDDS,DDGDDS,JCOBFZ,DDBDDS,
     & DSTRES,DDGDS,DDSTIFF,DDEDDB,Q,INVQ,R,NBLFMT,R0,NBLQR,ATRT,RB,IS,
     & JS,TSTRESS,PHI,KSTRESS,KDSTRES,MSTRESS,NG,NF,FB,FC,FK,HARDK,
     & KDNBL,NDNBL,IDNBL,TDNBL
C***FLD****FLD**************FLD************************
...
c*************************************************************
     ...
     & FYA,FYB,FYC,FYH,HARD0,HARD90,HARDB,HARD45)
...
C*********************************************************************	  
     ...
     &     (1-FD0)*(FE0+FF0*(1-EXP(-FG0*TDNBL)))
          SIGMA90K=FD90*FA90*(TDNBL+FB90)**FC90+
     &     (1-FD90)*(FE90+FF90*(1-EXP(-FG90*TDNBL)))  
          SIGMA45K=FD45*FA45*(TDNBL+FB45)**FC45+
     &     (1-FD45)*(FE45+FF45*(1-EXP(-FG45*TDNBL)))
     ...
     &     FYAK,FYBK,FYCK,FYHK,HARD0K,HARD90K,HARDBK,HARD45K)
          AAHK=w(1)
          BBHK=w(2)
          CCHK=w(3)
          HHHK=w(4)
...
C     CALCULATE G FUNCTION ON POINT C
          ...
          GTAO_SXY=GN*TAO_XY*GSUM
          GEQSTRESS=GF*(SIGMA_Y-SIGMA_Z)**TWO+GG*(SIGMA_Z-SIGMA_X)**TWO
     &     +GH*(SIGMA_X-SIGMA_Y)**TWO+TWO*GL*TAO_YZ**TWO+
     &     TWO*GM*TAO_ZX**TWO+TWO*GN*TAO_XY**TWO
          GEQSTRESS=DSQRT(THREE*GSUM*GEQSTRESS/TWO)
...
C      FIRST ORDER F-SIGMA
            ...
            ENDDO
            FPHI_S(1)=((FS1-FS2)*(ABS(FS1-FS2)**(FYM-2))-
     &       (FS3-FS1)*(ABS(FS3-FS1)**(FYM-2)))*FYM
     &       FPHI_S(2)=((FS2-FS3)*(ABS(FS2-FS3)**(FYM-2))-
     &       (FS1-FS2)*(ABS(FS1-FS2)**(FYM-2)))*FYM
     &       FPHI_S(3)=((FS3-FS1)*(ABS(FS3-FS1)**(FYM-2))-
     &       (FS2-FS3)*(ABS(FS2-FS3)**(FYM-2)))*FYM
C
            DO K1=1,NDI
	            DO K2=1,NDI
	              FS_I(K1,K2)=ZERO
...
C     SENCOND ORDER G-SIGMA ri
     ...
     1 GSIGMA_SX/(GEQSTRESS**3))
      DDBDDS(1,2)=ONEHALF*((-GH)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SX*GSIGMA_SY/(GEQSTRESS**3))
      DDBDDS(1,3)=ONEHALF*((-GG)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SX*GSIGMA_SZ/(GEQSTRESS**3))
      ...
      DDBDDS(2,2)=ONEHALF*((GF+GH)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SY*GSIGMA_SY/(GEQSTRESS**3))
	    DDBDDS(2,3)=ONEHALF*((-GF)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SY*GSIGMA_SZ/(GEQSTRESS**3))
      ...
      DDBDDS(3,1)=ONEHALF*((-GG)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SZ*GSIGMA_SX/(GEQSTRESS**3))
      DDBDDS(3,2)=ONEHALF*((-GF)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SZ*GSIGMA_SY/(GEQSTRESS**3))
	    DDBDDS(3,3)=ONEHALF*((GG+GF)*GSUM/GEQSTRESS-ONEHALF*GSIGMA_SZ*GSIGMA_SZ/(GEQSTRESS**3))
      ...
      DDBDDS(4,4)=THREE*(GN*GSUM/GEQSTRESS-THREE*GTAO_SXY*GTAO_SXY/(GEQSTRESS**3))
      ...
      DDBDDS(5,5)=THREE*(GM*GSUM/GEQSTRESS-THREE*GTAO_SZX*GTAO_SZX/(GEQSTRESS**3))
      ...
      DDBDDS(6,6)=THREE*(GL*GSUM/GEQSTRESS-THREE*GTAO_SYZ*GTAO_SYZ/(GEQSTRESS**3))
...
!------
     ...
     & hard90,hardb,hard45,R1,R2,R3,R4,TT 
      DO i=1,4
      f(i)=0
      ENDDO
...
c      write(*,*),mm,s00,aa,hard0,R1,R4,TT 
     ...
     & (abs(2*bb+cc))**(mm-2)*2+(bb+2*cc)*(abs(bb+2*cc))**(mm-2)

      df(1,3)=-(bb-cc)*(abs(bb-cc))**(mm-2)+(2*bb+cc)*
     & (abs(2*bb+cc))**(mm-2)+(bb+2*cc)*(abs(bb+2*cc))**(mm-2)*2

      df(1,4)=0
      
      df(2,1)=-(cc-aa)*(abs(cc-aa))**(mm-2)+(2*aa+cc)*
     & (abs(2*aa+cc))**(mm-2)*2+(aa+2*cc)*(abs(aa+2*cc))**(mm-2)
      
      df(2,2)=0
      
      df(2,3)=(cc-aa)*(abs(cc-aa))**(mm-2)+(2*aa+cc)*
     & (abs(2*aa+cc))**(mm-2)+(aa+2*cc)*(abs(aa+2*cc))**(mm-2)*2
      
      df(2,4)=0 
      
      df(3,1)=(aa-bb)*(abs(aa-bb))**(mm-2)+(2*bb+aa)*
     & (abs(2*bb+aa))**(mm-2)+(bb+2*aa)*(abs(bb+2*aa))**(mm-2)*2
      
      df(3,2)=-(aa-bb)*(abs(aa-bb))**(mm-2)+(2*bb+aa)*
     & (abs(2*bb+aa))**(mm-2)*2+(bb+2*aa)*(abs(bb+2*aa))**(mm-2)
     ...
     & (abs((aa+bb)/4-TT))**(mm-2)*(1/4-((aa-bb)/72)/(2*TT))+
     & ((aa+bb)/4+TT)*(abs((aa+bb)/4+TT))**(mm-2)*
     & (1/4+((aa-bb)/72)/(2*TT))

      df(4,2)=(2**mm)*TT**(mm-1)*(-(aa-bb)/72)/(2*TT)+
     & ((aa+bb)/4-TT)*(abs((aa+bb)/4-TT))**(mm-2)*
     & (1/4+((aa-bb)/72)/(2*TT))+mm*((aa+bb)/4+TT)*
     & (abs((aa+bb)/4+TT))**(mm-2)*(1/4-((aa-bb)/72)/(2*TT))

      df(4,3)=0

      df(4,4)=(2**mm)*TT**(mm-1)*(hh/2)/(2*TT)+((aa+bb)/4-TT)*
     & (abs((aa+bb)/4-TT))**(mm-2)*(-hh/2)/(2*TT)+((aa+bb)/4+TT)*
     & (abs((aa+bb)/4+TT))**(mm-2)*(hh/2)/(2*TT)
